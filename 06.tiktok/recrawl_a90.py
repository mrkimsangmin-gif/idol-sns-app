...
"""
recrawl_a90.py — A90 디바이스용 TikTok 재크롤링
S23에서 0~3개 게시물로 기록된 49개 그룹 재수집

A90 특이사항:
  - 해상도: 1080x2400 (S23: 1440x3088)
  - resource-id 체계 다름: yrd(그리드), r0f/r0e(프로필), r1p(유저네임)
"""
...
# ─── A90 디바이스 설정 ──────────────────────────────────────────────────────────
DEVICE = "RFCN30XWX8W"  # Samsung A90
...
DELAY = {
    "tap": 0.8,
    "transition": 2.0,
    "profile_load": 5.0,   # A90 더 느림 → 5초
    "video_load": 4.0,     # A90 더 느림 → 4초
    "scroll": 1.2,
    "back": 1.0,
    "between_groups": 3.0,
    "swipe": 1.5,
...
# ─── 재크롤링 대상 (name_en 기준) ─────────────────────────────────────────────
RECRAWL_TARGETS = [
    "BTS", "TWICE", "SEVENTEEN", "NewJeans", "IVE", "CORTIS", "TREASURE",
    "ASTRO", "NMIXX", "ZEROBASEONE", "MAMAMOO", "Red Velvet", "CRAVITY",
    "KISS OF LIFE", "Ikon", "UNIS", "LNGSHOT", "WEi", "WOOAH", "XLOV",
    "MEOVV", "FIFTY FIFTY", "8TURN", "fromis_9", "ARTBEAT v", "EPEX",
    "Billlie", "Xdinary Heroes", "VIVIZ", "ONEUS", "PENTAGON", "TEMPEST",
    "Dreamcatcher", "ALLDAY PROJECT", "E'LAST", "MIRAE", "xikers",
    "Weeekly", "KiiiKiii", "THE NEW SIX", "BLITZERS", "CIX", "BLACKSWAN",
    "THE BOYZ", "ATBO", "ALL(H)OURS", "YOUNITE", "TRENDZ", "PURPLE KISS",
]

# ─── 로깅 ─────────────────────────────────────────────────────────────────────
LOG_DIR = SCRIPT_DIR / "logs"
LOG_DIR.mkdir(exist_ok=True)
log_file = LOG_DIR / f"recrawl_a90_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def swipe_up():
    # A90: 1080x2400
    adb(f"input swipe 540 1700 540 600 300")
...
def wake_screen():
    ...
    if "NotificationShade" in focus or "Keyguard" in focus or "StatusBar" in focus or "Launcher" in focus:
        ...
        time.sleep(1)
        adb("input swipe 540 2000 540 500 500")  # 잠금해제 (긴 스와이프)
        time.sleep(2)
    ...
    try:
        ...
        if not grid_positions:
            for grid_retry in range(4):
                ...
                adb(f"input swipe 540 1400 540 1200 200")
                time.sleep(wait_sec)
...
def main():
    ...
    if "ok" not in r.stdout:
        ...
        sys.exit(1)
    logger.info(f"디바이스 연결 확인: {DEVICE} (SM-A9080)")
